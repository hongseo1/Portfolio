...
//슬라이드 이미지
    ...
    function timer(){ //일정시간 호출하기위해 사용자 지정 함수 타이머를 만듬

        setIntervalId = setInterval(function(){ //반복실행을 지정하는 부분

            var prev = visual.eq(current); //현재 자기자신의 위치를 prev에 담고
            var pn = button.eq(current); //현재 순선 버튼을 pn으로 담고
            ...
            //비주얼 이미지의 사이즈가 끝까지 갔다면(끝번호라면) current에 담아서 0으로 초기화

            var next = visual.eq(current);
            var pnn = button.eq(current);
            move(next, '100%', 0); //100%에서 0%로
            pnn.addClass('on')
        },3000);
    ...
    button.click(function(){
        ...
        var i = tg.index(); //클릭되어진 요소(인덱스 0~2)를 i라 하고
        button.removeClass('on'); //on 스타일을 해제
        tg.addClass('on'); //클릭되어진 요소의 on스타일 활성화
        movel(i); //버튼을 누르면 현재 화면에서 해당요소의 이미지가 보여지도록

    });

    function movel(i){ 
        if(current==i){return} //i가 현재화면과 같다면 되돌아가도록
...
//보도자료 슬라이드
    ...
    function set(){ //set객체를 만듬.
        tim = setInterval(function(){ //반복실행된는 서브텍스트들을 tim으로 지정

            var prev1=subtext.eq(current1); //현재 초기화된 서브텍스트를 prev1에 저장
            move1(prev1, 0, '-100%'); //위로이동

            current1++; //하나씩 추가되어 보여지도록

            if(current1==subtext.size()){ //서브텍스트의 순번이 가득 찼다면 다시 0번으로
                current1=0;
            }
    ...
    $('.sub_text').hover(function(){ //마우스를 올리면
...
//사회공헌 슬라이드
    ...
    function set1(){ //set객체를 만듬.
        ...
        tim1 = setInterval(function(){ //반복실행된는 서브텍스트들을 tim으로 지정

            var prev2=subtext1.eq(current2); //현재 초기화된 서브텍스트를 prev1에 저장
            move2(prev2, 0, '-100%'); //위로이동

            current2++; //하나씩 추가되어 보여지도록

            if(current2==subtext1.size()){ //서브텍스트의 순번이 가득 찼다면 다시 0번으로
                current2=0;
            }
    ...
    $('.sub_text1').hover(function(){ //마우스를 올리면
        clearInterval(tim1); //반복실행 해제
    },function(){ //마우스를 떼면
        set1(); //set함수 다시 불러와 실행
    });
...
//top버튼
    $('#top_btn>a').hide();
        $(window).scroll(function(){ //브라우저에 스크롤 변화가 발생하면 호출
            if($(this).scrollTop()>500){ //(scrollTop 수직위치) 스크롤의 위치가 500보다 크다면
...
//모바일 메뉴
    ...
    $('.mo_depth2').hide(); //서브메뉴 처음에는 안 보이게
